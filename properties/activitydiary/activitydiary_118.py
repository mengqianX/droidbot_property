...
class Test(AndroidCheck):
    ...
    @precondition(
        lambda self: self.device(text="Diary").exists() and self.device(resourceId="de.rampro.activitydiary.debug:id/picture").exists()
    )
    @rule()
...
t = Test(
    ...
    device_serial="emulator-5554",
    output_dir="output/activitydiary/118/random_100/1",
    policy_name="random",
    timeout=21600,
    number_of_events_that_restart_app = 100
)
