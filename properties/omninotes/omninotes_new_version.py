...
class Test(AndroidCheck):
    ...
    def rule_trash_note(self):
        ...
        self.device(text="Trash").click()
        time.sleep(1)
        assert self.device(resourceId="it.feio.android.omninotes:id/list").child_by_text(selected_note_name,allow_scroll_search=True,resourceId="it.feio.android.omninotes:id/note_title").exists()

    # bug #277
    @precondition(lambda self: self.device(text="Search in notes").exists() and self.device(resourceId="it.feio.android.omninotes:id/menu_tags").exists() and not self.device(text="Settings").exists())
    @rule()
    def rule_search_by_tag_should_display_results(self):
        ...
        selected_tag_name = self.device(resourceId="android:id/text1")[selected_tag].info['text']
...
t = Test(
    ...
    device_serial="emulator-5554",
    output_dir="output/omninotes/new/1",
