...
class Test(AndroidCheck):
    ...
    def set_up(self):
        ...
        # self.device(resourceId="nl.mpcjanssen.simpletask:id/editText").set_text(tag_name)
        # time.sleep(1)
        # self.device(text="OK").click()
        # time.sleep(1)
    ...
    @precondition(
        lambda self: self.device(resourceId="nl.mpcjanssen.simpletask:id/btnContext").exists()
    )
    @rule()
    def add_tag_the_cursor_should_after_the_tag(self):
        self.device(resourceId="nl.mpcjanssen.simpletask:id/btnProject").click()
        time.sleep(1)
        tag_name = st.text(alphabet=string.ascii_letters,min_size=1, max_size=6).example()
        print("tag name: "+str(tag_name))
        ...
        self.device(text="OK").click()
        time.sleep(1)
        self.device(resourceId="com.google.android.inputmethod.latin:id/label_bottom").click()
        time.sleep(1)
        content = self.device(resourceId="nl.mpcjanssen.simpletask:id/taskText").get_text()
        print("content: "+str(content))
        assert tag_name in content, "tag name should in content"
        assert content[-1] == ",", "the cursor should after the tag"
...
# )
t = Test(
    apk_path="./apk/simpletask/7.2.0.apk",
    device_serial="emulator-5554",
    output_dir="output/simpletask/677/random_100/1",
    policy_name="random",
    timeout=21600,
    number_of_events_that_restart_app = 100
)
