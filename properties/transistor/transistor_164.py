...
class Test(AndroidCheck):
    ...
    def rename_station_should_work(self):
        ...
        time.sleep(1)
        assert self.device(text=new_name).exists(), "NEW NAME DOES NOT EXIST after playback"
        assert not self.device(text=station_name).exists(), "OLD NAME STILL EXISTS after playback"
