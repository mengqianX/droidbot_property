...
class UIEmbedLSTM(nn.Module):
    ...
    def _encode_view(self, state, view):
        ...
        long_clickable = (
            1 if 'long_clickable' in view and view['long_clickable'] else -1
        )
...
class Memory:
    ...
    def save_transition(self, action, from_state, to_state):
        ...
        if (
            action_str in self.known_transitions
            and self.known_transitions[action_str]['to_state'] == to_state
        ):
            return
    ...
    def _select_transitions_for_training(self, size):
        ...
        selected_actions = np.random.choice(
            action_strs, size=size, replace=False, p=action_probs
        )
    ...
    def encode_action_pairs(self, action_strs=None):
        ...
        state_strs = [
            self.known_transitions[action_str]['from_state'].state_str
            for action_str in action_strs
        ]
        state_encs = [
            self.known_states[state_str]['state_enc'] for state_str in state_strs
        ]
        action_pairs = []
        for i, action_str1 in enumerate(action_strs):
            ...
            for j, action_str2 in enumerate(action_strs[i + 1 :]):
                ...
                action_pairs.append(
                    (state_idx1, view_idx1, state_idx2, view_idx2, effect_same)
                )
    ...
    def train_model(self):
        ...
        for i in range(n_iterations):
            ...
            print(
                f'| iter: {i:3d} | time: {elapsed:6.2f}s | #pairs: {n_pairs:6d} | loss: {loss:8.4f}'
            )
...
class MemoryGuidedPolicy(UtgBasedInputPolicy):
    ...
    def generate_event_based_on_utg(self):
        ...
        if (
            self.action_count >= self.num_actions_train
            ...
            if target_state:
                ...
                if target_state.state_str == current_state.state_str:
                    self.logger.info(
                        f"executing action selected from {len(candidates)} candidates"
                    )
                    return target_action
                self._nav_steps = self.get_shortest_nav_steps(
                    current_state, target_state, target_action
                )
                nav_action = self.navigate(current_state)
                if nav_action:
                    return nav_action
    ...
    def pick_target(self, current_state):
        ...
        if DEBUG:
            known_actions_str = [
                Memory.action_info_str(v)
                for k, v in self.memory.known_transitions.items()
            ]
        scores = []
        for i, (state, action) in enumerate(state_action_pairs):
            action_emb = self.memory.get_action_emb(state, action)
            similarities = torch.cosine_similarity(
                action_emb.repeat((known_actions_emb.size(0), 1)), known_actions_emb
            )
            max_sim, max_sim_idx = similarities.max(0)
            score = -max_sim
            if state.state_str == current_state.state_str:
                score += (
                    CLOSER_ACTION_ENCOURAGEMENT  # encourage actions in current state
                )
            if DEBUG:
                action_info_str = (
                    f'{state.foreground_activity}-{action.view["signature"]}'
                )
    ...
    def get_shortest_nav_steps(self, current_state, target_state, target_action):
        ...
        restart_nav_steps = self.utg.get_G2_nav_steps(
            self.utg.first_state, target_state
        )
        normal_nav_steps_len = (
            len(normal_nav_steps) if normal_nav_steps else MAX_NAV_STEPS
        )
        restart_nav_steps_len = (
            len(restart_nav_steps) + 1 if restart_nav_steps else MAX_NAV_STEPS
        )
        if (
            normal_nav_steps_len >= MAX_NAV_STEPS
            and restart_nav_steps_len >= MAX_NAV_STEPS
        ):
            self.logger.warning(
                f'cannot find a path to {target_state.structure_str} {target_state.foreground_activity}'
            )
            ...
            for action_str in self.memory.known_transitions:
                action_from_state = self.memory.known_transitions[action_str][
                    'from_state'
                ]
                action_to_state = self.memory.known_transitions[action_str]['to_state']
                if (
                    action_from_state.state_str == target_state_str
                    or action_to_state.state_str == target_state_str
                ):
        ...
        # elif normal_nav_steps_len > restart_nav_steps_len:  # prefer shortest path
            ...
            nav_steps = [
                (current_state, KillAppEvent(app=self.app))
            ] + restart_nav_steps
        else:
