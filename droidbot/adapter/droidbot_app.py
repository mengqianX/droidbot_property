...
class DroidBotAppConn(Adapter):
    ...
    def set_up(self):
        ...
        if (
            ACCESSIBILITY_SERVICE not in device.get_service_names()
            and self.device.get_sdk_version() < 23
            and self.enable_accessibility_hard
        ):
        ...
        while (
            ACCESSIBILITY_SERVICE not in device.get_service_names() and self.__can_wait
        ):
    ...
    def connect(self):
        ...
        try:
            ...
            forward_cmd = "adb %s forward tcp:%d %s" % (
                serial_cmd,
                self.port,
                DROIDBOT_APP_REMOTE_ADDR,
            )
    ...
    def handle_message(self, message):
        ...
        if acc_event_idx >= 0:
            if acc_event_idx > 0:
                self.logger.warning(
                    "Invalid data before packet head: " + message[:acc_event_idx]
                )
        ...
        if rotation_idx >= 0:
            if rotation_idx > 0:
                self.logger.warning(
                    "Invalid data before packet head: " + message[:rotation_idx]
                )
    ...
    def disconnect(self):
        ...
        if self.sock is not None:
            try:
                ...
                print(e)
        try:
            forward_remove_cmd = "adb -s %s forward --remove tcp:%d" % (
                self.device.serial,
                self.port,
            )
            p = subprocess.Popen(
                forward_remove_cmd.split(),
